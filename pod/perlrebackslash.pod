...
C<\b{...}> matches a boundary (between two characters, or before the
first character of the string, or after the final character of the
string) based on the Unicode rules for the boundary type specified
inside the braces.  The currently known boundary types are given a few
paragraphs below.  C<\B{...}> matches at any place between characters
where C<\b{...}> of the same type doesn't match.
...
available is C<gcb> (a short form is also legal: C<g>), which stands for
"Grapheme Cluster Break".  (Actually Perl always uses the improved
